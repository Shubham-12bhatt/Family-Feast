import React from 'react';
import {
  Facebook,
  Instagram,
  Twitter,
  Youtube,
  Phone,
  Mail,
} from "lucide-react";
import { MapPin } from 'lucide-react';

const Footer = () => {
  return (
    <footer className="bg-[#7A2E45] text-white pt-16 pb-8 px-6 md:px-16 border-t border-white/10">
      <div className="container mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-5 mb-12">
        <div className="space-y-4">
          <h3 className="text-3xl font-poppins font-bold italic text-white">Family Feast</h3>
          <p className="text-white font-poppins text-sm leading-relaxed">
            Leading rice exporter committed to quality, sustainability, and customer satisfaction.
          </p>
          <div className="flex gap-4 pt-2">
            {/* Social Icons Placeholder */}
            <div className="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center hover:bg-secondary hover:text-[#7A2E45] transition-colors cursor-pointer bg-white/10 hover:bg-white">
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
            </div>
            <div className="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center hover:bg-secondary hover:text-[#7A2E45] transition-colors cursor-pointer bg-white/10 hover:bg-white">
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" /></svg>
            </div>
          </div>
        </div>

        <div>
          <h4 className="text-lg font-bold mb-4 font-poppins lg:ml-8">Quick Links</h4>
          <ul className="space-y-3 font-poppins text-white text-sm lg:ml-8">
            {["Home", "About Us", "Our Products", "Contact Us"].map((item) => (
              <li key={item}>
                <a
                  href="#"
                  className="flex items-center gap-2 hover:text-gray-300 transition-colors"
                >
                  <span className="text-white text-md mr-1.5">&gt;</span>
                  {item}
                </a>
              </li>
            ))}
          </ul>
        </div>

        <div>
          <h4 className="text-lg font-bold mb-4 font-poppins">Products</h4>
          <ul className="space-y-3 font-poppins text-white text-sm">
            {["Basmati Premium Rice", "Basmati White Sella", "Basmati Golden Sella"].map((item) => (
              <li key={item}>
                <a
                  href="#"
                  className="flex items-center gap-2 hover:text-secondary transition-colors"
                >
                  <span className="text-white text-md mr-1.5">&gt;</span>
                  {item}
                </a>
              </li>
            ))}
          </ul>
        </div>


        {/* Address */}
        <div>
          <h4 className="text-lg font-bold mb-4 font-poppins text-white">
            Address
          </h4>

          {/* Address */}
          <div className="flex items-center gap-2.5 mb-3 ">
            <MapPin className="w-8 h-8 text-white " />
            <p className="text-white text-sm font-poppins leading-relaxed ">
              123, Rice Market, Grain Street, Ahmedabad – 380001
            </p>
          </div>

          {/* Email */}
          <div className="flex items-center gap-3 mb-3">
            <Mail className="w-4 h-4 text-white " />
            <p className="text-white text-sm font-poppins">
              <span className="font-semibold">Email:</span> info@familyfeast.com
            </p>
          </div>

          {/* Phone */}
          <div className="flex items-center gap-3">
            <Phone className="w-4 h-4 text-white" />
            <p className="text-white text-sm font-poppins">
              <span className="font-semibold">Phone:</span> +91 98765 43210
            </p>
          </div>
        </div>


      </div>

      <div className="container mx-auto px-6 border-t-2 border-white/20 pt-8 flex flex-col md:flex-row justify-between items-center text-md text-white-400 font-poppins">
        <p>&copy; 2026 Family Feast. All Rights Reserved.</p>

      </div>
    </footer>
  );
};

export default Footer;
